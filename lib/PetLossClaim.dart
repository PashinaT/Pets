...
class PetLossClaim extends StatefulWidget {
  PetLossClaim(
  ...
  State<StatefulWidget> createState() {
    return PetLossClaimForm(title, listPet, isUpdating, isFoundPage);
  }
}

class PetLossClaimForm extends State<PetLossClaim> {
  final String title;
  int id;
  String namePet;
  String colorPet;
  ...
  PetLossClaimForm(this.title, this.listPet, this.isUpdating, this.isFoundPage);
  ...
  void initState() {
    ...
    dbHelper.getShelter().then((row) {
      row.map((map) {
        ...
        shelterList.add(dropDownItems);
      });
      setState(() {});
    });
    namePetController.text = listPet == null
        ? namePet == null
        ? "" : namePet
        : listPet.namePet;
  ...
  validate() {
    if (formKey.currentState.validate()) {
      ...
      if (isUpdating) {
        ...
        ListPets e = ListPets(null, namePet, colorPet, viewPet, isFoundPage,base64Image,null);
          ...
          child: Padding(
              ...
              child: Column(
                  ...
                  children: <Widget>[
                    ...
                    DropdownButtonFormField(
                      onChanged: (value) {},
                      onSaved: (val) => listPet ==null ? {} :listPet.shelter = val,
                      validator: (val) => val == null ? 'Выберите приют' : null,
                      decoration: InputDecoration( hintText: 'В каком приюте? ',
                        suffixIcon: IconButton(
                          icon: Icon(Icons.clear),
                          onPressed: () {
                            formKey.currentState.reset();
